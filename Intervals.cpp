...
FixedWrapper FixedWrapper::CalcPi(uint32_t prec = FixedWrapper::CALCPRECISION) {
    ...
    while (s != lasts) {
        ...
        t = t * n / d;
        // n / d = (2i - 1) ^ 2 / (8i * (2i + 1)) <= 1 / 4
        // Деление вычисляется с потерей точности в 1 единицу младшего разряда
        // max(t, 1) младшего разряда точности потеряно при умножении, но при prec >= 1 верно:
        // t_1 = (t_0 * (n_1 / d_1) + max(t_0/10^prec, 1/10^prec));
        // t_0 = 3 => l_1 = 3 единицы точности потеряно
        // t_2 = (t_1 * (n_2 / d_2) + max(t_1/10^prec, 1/10^prec)); 
        // t_1 = 0.125 => l_2 = l_1 * (n_2 / d_2) + 1 < 3 / 4 + 1 < 2 единицы точности потеряно
        // В общем случае для k > 1 верно l_k = l_k-1 * (n_k / d_k) + 1
        // Видно, что по индукции потеря на каждом конкретном слагаемом не более 2 единиц младшего разряда
        // (Кроме второго слагаемого (t1), которое приводит к потере не более чем 3 единиц точности)
        s += t;
    }  
    // t[i] < t[i - 1] / 4
    // При этом количество итераций i <= [prec * log4 10] + 4 <= [prec * 5 / 3] + 4
    // (Здесь [] - взятие целой части)
    // (Константа +4 выбрана так, чтобы компенсировать округление до целого вниз)
    // Тогда max потерь = MLoss(prec) <= 2 * i + 1 <= 2 * ([prec * 5 / 3] + 4) + 1 <= [prec * 10 / 3] + 9
    // (+1 - лишняя потеря единицы точности от первого слагаемого, описанного выше)
    // Считаем, что нужно потерять не более 1 единицы вычислительной точности (calcprec)
    // Допустим мы считаем с итоговой точностью prec
    // Тогда условие потери 1 единицы точности: MLoss(prec) <= 10 ^ ([prec - calcprec])
    // Выберем prec = [calcprec * 1.1] + 9
    // [([calcprec * 1.1] + 9) * 10 / 3] + 9 > 10 ^ ([0.1 * calcprec] + 9)
    // Неравенство неразрешимо при неотрицательном calcprec
    // (Константа 9 позволяет гарантировать условие, если при вычислении prec округляется вниз до целого)
    // Значит prec = calcprec * 1.1 + 9 достаточно для вычисления Пи с точностью calcprec
